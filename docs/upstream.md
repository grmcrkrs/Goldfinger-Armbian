...
The tested implementation is maintained on a `goldfinger-v14` branch based on
ophub commit `26ce84cdd716b307517e794880666ebb3ca19944`. Upstream review should be
split so maintainers can evaluate reusable framework changes independently from
this board.
...
The submission must state that qualification covers two boards with the exact
PCB marking/date and remains experimental community support.
...
and generates U-Boot scripts locally. Public binary-image upload remains gated
by `docs/redistribution.md`; source review does not require committing the blobs.
